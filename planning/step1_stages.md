...
- Додати loader для `client_profiles.manual.csv`.
- Додати loader для `application_history.manual.csv`.
- Визначити idempotency strategy для application history, щоб повторний import не створював дублікати.
- Додати CLI commands для import:
  - `grant-tool import-clients`;
  - `grant-tool import-application-history`.
- Додати tests для CSV parsing, validation і repeated import.
