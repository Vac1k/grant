...
Великий Stage Search завершений.

Фактичний стан: реалізована базова інфраструктура search/link extraction, адаптація 4 MVP-джерел, аудит усіх наданих джерел, standardized initial discovery table, правила початкового наповнення, Step 4.1/4.2/4.3 для нових джерел, Step 5 real website validation, Step 6 incremental behavior, Step 7 refresh policy, Step 8 CLI operational visibility, Step 9 production backfill/quality gate, re-audit для `grantforward`/`grantsense`, і Step 10 фінальне документування.

Stage Search закритий, бо виконані всі вимоги:
...
- `plan_for_search.md` більше не містить відкритих implementation steps для Stage Search.
...
Фінальний Stage Search status:

- real website validation виконано;
- source входить у production backfill і quality gate;
- live quality gate підтвердив мінімум 10 quality-approved records;
- status/deadline зберігаються з API payload і проходять через normalized grant fields.
...
- source входить у production backfill і quality gate;
- live quality gate підтвердив мінімум 10 quality-approved records;
- detail pages дають достатньо тексту для normalized grant records.
...
- source входить у production backfill і quality gate;
- live quality gate підтвердив мінімум 10 quality-approved records;
- source зберігається як broader business finance/support source з manual-review контекстом.
...
- connector реалізований у codebase;
- production validation для правильного URL `https://gurt.org.ua/news/grants/` заблокована Cloudflare/human-check;
- source не входить у обов'язкову вимогу 10 quality-approved records;
- documented access limitation закриває source без bypass.
...
- `grantforward` - у Step 4.3 був задокументований як deferred/restricted; після пізнішого re-audit нижче додано робочий connector через public search AJAX.
...
### GrantForward Deferred У Step 4.3
...
Рішення на момент Step 4.3: не додано як робочий connector.
...
Це рішення було змінено після повторного аудиту: знайдено дозволений public AJAX endpoint `/search/search`, тому `grantforward` тепер є implemented source з обмеженням login-only detail pages.
...
- на момент Step 4.3 `grantforward` не було додано в seed, бо джерело вважалось deferred/restricted;
- після re-audit `seed-sources` створює або оновлює 12 configured sources, включно з `grantforward`.

Оновлено connector registry:

- `CONNECTOR_CLASSES["opportunitydesk"]`;
- після re-audit також `CONNECTOR_CLASSES["grantforward"]`.
...
| `grantforward` | `https://www.grantforward.com/search` | `200 text/html`; пізніше підтверджено public AJAX endpoint `/search/search` з result HTML. | `implemented_with_limitations_after_reaudit` |
...
### Зафіксовані Production Limitations І Рішення

| Source slug | Limitation | Stage Search decision |
|---|---|---|
| `eu-funding` | Live API can return raw status-like numeric values. | Source accepted because title, URL, deadline/status payload and quality records are present; raw status value remains traceable in stored payload. |
| `diia-business` | Source includes broader business finance/support programmes, not only pure grants. | Source accepted with `support_type` and manual review/feature extraction checks. |
| `gurt` | Cloudflare/human-check returns `403 Forbidden`. | Source closed as documented access limitation; no bypass. |
| `eufundingportal-eu` | Aggregator source can duplicate official EU Funding opportunities. | Source accepted with duplicate risk metadata and manual review. |
| `hromady` | Sample can be digest/list content, not always direct grant detail. | Source accepted with quality gate/manual review filtering. |
| `nipo` | News/digest source; sample can be webinar/digest content. | Source accepted with `needs_manual_review`. |
| `fundsforngos` | Broad international source with country/topic mismatch risk. | Source accepted with `needs_manual_review` and strong filters. |
| `opportunitydesk` | Broad opportunity source; sampled item was already `closed`, which is allowed because search does not use active-only filter. | Source accepted with digest filter and manual review. |
| `grantsense` | No stable direct opportunity feed confirmed. | Source closed as deferred/blocked. |
| `grantforward` | Detail pages залишаються login-only; public search endpoint дає тільки першу сторінку результатів. | Source accepted with public search fields only and manual review. |
...
- `grantsense` має documented deferred/blocked evidence;
- `grantforward` мав deferred/restricted evidence на момент Step 5, але після re-audit реалізований через public search AJAX;
...
| `grantforward` | 10 | `success` | 10 | 10 | 0 | 0 |

### Final Quality Gate Result

```text
Search quality gate: passed (11/11 required sources passed)
```
...
| `grantforward` | 10 / 10 | 10 | 0 | `passed_with_login_only_detail_limitation` |
...
- `grantforward` дає тільки першу public search page без login; `limit > 10` повертає login/sign-up message, тому production backfill для цього source має використовувати `limit=10`.
...
### Re-Audit: GrantForward І GrantSense

Після окремого запиту на покращення для `grantsense` і `grantforward` виконано повторний live-аудит.

#### GrantForward

Статус змінено з `deferred/restricted` на `implemented_with_limitations`.

Що реалізовано:

- додано source `grantforward` у `SOURCE_DEFINITIONS`;
- додано `GrantForwardConnector`;
- додано registry entry `CONNECTOR_CLASSES["grantforward"]`;
- connector використовує public AJAX endpoint `https://www.grantforward.com/search/search`;
- перед AJAX-запитом connector відкриває `https://www.grantforward.com/search`, щоб працювати так само, як public search UI;
- search query налаштований на `ukraine`;
- нормалізуються public search fields:
  - `source_record_id` з `js-result-id` / `grant_id`;
  - `title` з `.grant-url`;
  - `summary` з `.description`;
  - `funder_name` і список `sponsors` з `.sponsor a`;
  - `deadline_text` з deadline table або `.deadline-value`;
  - `deadline_at` через deterministic date parser;
  - `funding_amount_text` з `.amount` / `aria-label`;
  - `source_url` як GrantForward detail URL;
- detail pages не scrape-яться, бо live check показав redirect на login;
- кожен record має `needs_manual_review=True`, бо detail-only fields недоступні без login;
- `grantforward` додано до quality gate як structured grant-search source.
...
```text
Seeded 12 configured sources
grantforward: success processed=10 created=10 updated=0 skipped=0 failed=0 errors=0
Search quality gate: passed (11/11 required sources passed)
grantforward | yes | 10/10 | 10 | 0 | passed
```

Важливе обмеження:

- `limit > 10` для live search повернув повідомлення login/sign-up;
- це не обходиться, бо це вже access restriction;
- для `grantforward` production command має використовувати `--limit 10`;
- якість gate досягається тим, що `grantforward` є structured grant-search source, а не generic HTML page.

#### GrantSense

Статус лишається `deferred/blocked`.

Повторна live-перевірка:

```text
https://www.grantsense.com.ua/grants2025 -> 500 text/html
https://www.grantsense.com.ua/grants2024 -> 500 text/html
sample blog detail -> 500 text/html
https://webcms.grantsense.com.ua/graphql -> timeout / no response
https://webcms.grantsense.com.ua/api/grants?populate=* -> timeout / no response
https://webcms.grantsense.com.ua/api/blogs?populate=* -> timeout / no response
https://www.grantsense.com.ua/sitemap.xml -> 200 application/xml
```

Чому connector не додано:

- sitemap сам по собі містить переважно service/category/blog URLs;
- direct grant opportunity detail pages стабільно не відкриваються;
- CMS GraphQL/API не дає доступного public extraction surface;
- homepage містить частину serialized marketing/blog data, але це не є якісним feed грантових можливостей;
- додавання такого source у `grants` створило б noise замість якісних records.
...
- `grantsense` лишається deferred/blocked без stable connector;
- `grantforward` реалізовано через public search AJAX endpoint і додано до quality gate;
...
## Реалізовано: Step 10 - Фінальна Документація І Закриття Stage Search

Статус: виконано.

Дата: `2026-05-24`.

Step 10 закрив documentation state для всього Stage Search.
...
- `docs/start.md` - містить актуальні ingestion commands для всіх реалізованих джерел.

Фінально закрито:

- усі provided links проаудитовані;
- усі implementable sources мають source-specific connector;
- `gurt` закритий documented Cloudflare/human-check limitation;
- `grantsense` закритий documented deferred/blocked reason;
- `grantforward` реалізований через public search AJAX з login-only detail limitation;
- кожне implementable джерело, крім `gurt`, має мінімум 10 quality-approved records у `grants`;
- incremental new-only behavior реалізовано через `discovered_grant_items`;
- refresh policy зафіксована;
- CLI operational visibility і quality gate реалізовані.

Фінальна перевірка Step 10:
